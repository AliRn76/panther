...
class TestRun(TestCase):
    ...
    def test_load_configs(self):
        ...
        assert config['middlewares'][0].__class__.__name__ == 'DatabaseMiddleware'
        assert config['middlewares'][0].url == 'pantherdb://test.pdb'

        assert len(config['reversed_middlewares']) == 1
        assert config['reversed_middlewares'][0].__class__.__name__ == 'DatabaseMiddleware'
        assert config['reversed_middlewares'][0].url == 'pantherdb://test.pdb'
