...
file = File(file_name='document.pdf', content_type='application/pdf', file=b'PDF content here...')

# Create image file
image = Image(file_name='photo.jpg', content_type='image/jpeg', file=b'JPEG content here...')
```
...
async def upload_file(request: Request):
    ...
    # Store in database
    document = await Document.insert_one({'title': file_data.description, 'file': file, 'uploaded_at': datetime.now()})
