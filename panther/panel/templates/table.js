...
function formatValue(value, type) {
  ...
  const accordionId = `accordion-${Math.random()
    .toString(36)
    .substr(2, 9)}`;
...
`;
  ...
  // Then handle records if they exist
  const records = Array.from(
    document.querySelectorAll('.record-data')
  ).map((item) => JSON.parse(pythonToJSON(item.textContent.trim())));
